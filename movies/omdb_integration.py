...
# from .signals import movie_filled
...
def fill_movie_details(movie):
    ...
    # Dispatch signal
    movies.signals.movie_filled.send(sender=fill_movie_details, movie=movie)
...
def search_and_save(search):
    ...
    if not created and (search_term.last_search > now() - timedelta(days=1)):
        # Don't search as it has bean searched recently
        logger.warning(
            "Search for '%s' was performed in the past 24 hours so not searching again", normalized_search_term,)
        return
